...
import { Mic, TimerReset, ShieldAlert, Users, Map, Activity } from "lucide-react";
import { timeOfDayRisk } from "@/lib/sample-data";

export const Route = createFileRoute("/_authenticated/dashboard")({ component: Dashboard });

type Counts = {
  voice: number; sos: number; contacts: number; routes: number; guardianOn: boolean;
};
...
function Dashboard() {
  const [counts, setCounts] = useState<Counts>({
    voice: 0, sos: 0, contacts: 0, routes: 0, guardianOn: false,
  });
  ...
  useEffect(() => {
    void (async () => {
      ...
      const [v, s, c, r, settings] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mt-3">
        ...
        <Link to="/check-in" className="contents">
          <Stat icon={TimerReset} label="Check-in" value="Set" tone="navy" />
        </Link>
      </div>
